...
function template_cleanup() {
    ...
    add_filter( 'tiny_mce_plugins', 'template_disable_emojis_tinymce' );
    add_filter( 'wp_resource_hints', 'template_disable_emojis_remove_dns_prefetch', 10, 2 );
...
add_action( 'init', 'template_cleanup' );

//more emoji 
function template_disable_emojis_tinymce( $plugins ) {    if ( is_array( $plugins ) ) {    return array_diff( $plugins, array( 'wpemoji' ) );    } else {    return array();    }}

//more emoji 2
function template_disable_emojis_remove_dns_prefetch( $urls, $relation_type ) {
...
function template_show_less_login_info() { 
    return "<strong>ERROR</strong>: Stop guessing!"; }
add_filter( 'login_errors', 'template_show_less_login_info' );

 
// Do not generate and display WordPress version
function template_no_generator()  {     return ''; }
add_filter( 'the_generator', 'template_no_generator' );
...
add_action( 'init', 'template_disable_embeds_code_init', 9999 );

function template_disable_embeds_code_init() {
	...
	add_filter( 'tiny_mce_plugins', 'template_disable_embeds_tiny_mce_plugin' );
	
	// Remove all embeds rewrite rules.
	add_filter( 'rewrite_rules_array', 'template_disable_embeds_rewrites' );
	
	// Remove filter of the oEmbed result before any HTTP requests are made.
	remove_filter( 'pre_oembed_result', 'wp_filter_pre_oembed_result', 10 );
}

function template_disable_embeds_tiny_mce_plugin($plugins) { return array_diff($plugins, array('wpembed')); }

function template_disable_embeds_rewrites($rules) {
...
function template_wp_remove_global_css() {
   remove_action( 'wp_enqueue_scripts', 'wp_enqueue_global_styles' );
   remove_action( 'wp_body_open', 'wp_global_styles_render_svg_filters' );
}
add_action( 'init', 'template_wp_remove_global_css' );
